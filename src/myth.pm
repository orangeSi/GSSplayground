...
#print "fontsize is $fontsize before, height is $height\n";
	...
	eval "use $pkg";
	if($@){ # not find $pkg
		my $json_file = "$Bin/text_height_width_by_size.json";
		if(-f "$json_file"){
			print "using $json_file instead of $pkg\n"
		}else{
			die "error: no pkg $pkg && no json_file $json_file\n";
		}
		open IN,"$json_file" or die "error: not open $json_file\n";
		...
		die "error: it's a bug! fontsize $fontsize not exists in $json_file\n" if(not exists $myHashEncoded{$fontsize});
		if($type eq "height"){
			return $myHashEncoded{$fontsize}{height}{min} if($value eq "min");
			return $myHashEncoded{$fontsize}{height}{max}  if($value eq "max");
			return $myHashEncoded{$fontsize}{height}{avg};
		}elsif($type eq "width"){
			return $text_length * $myHashEncoded{$fontsize}{width};
		}else{
