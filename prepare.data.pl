	...
	for my $k (@{$conf->{reads_mapping}}){
		...
		for my $block_index(keys %{$gff->{$sample}->{chooselen_single}}){
			...
			for my $rg(@start_end_xaxis){
				...
				@outname = &write_gff_conf_link($prefix,$mapping_gff,$mapping_setting_conf,$cross_link_conf, \@outname);
			}
...
sub write_gff_conf_link(){
	my ($prefix,$gff,$setting_conf,$cross_link_conf, $outname)=@_;
	my @outnames=@$outname;
	...
	for my $k (@{$conf->{plot_depth}}){
		...
		for my $block_index(keys %{$gff->{$sample}->{chooselen_single}}){
			...
			for my $rg(@start_end_xaxis){
				...
				@outname = &write_gff_conf_link($prefix,$depth_gff,$depth_setting_conf,$cross_link_conf, \@outname);
			}
		}
	}
	for my $s(keys %outname){
	...
	for my $read_id(sort {$reads{$a}{ref_start}<=>$reads{$b}{ref_start}} keys %reads){
		...
		if($read_type eq "short_reads"){
			...
			for my $cr(keys %{$reads{$read_id}{cigar}}){
				$feature_color=&cs_color($reads{$read_id}{$cr}{type});
				$map_pos_start_cr=$reads{$read_id}{$cr}{start};
				$map_pos_end_cr=$reads{$read_id}{$cr}{end};
				$map_pos_strand_cr=$reads{$read_id}{$cr}{strand};
...
sub read_depth_file(){
	...
	if($depth_file=~ /.bam\s*$/){
		...
		if(! -f "$bam_depth_file"){
			...
			my $cmd="samtools depth  -r $scf:$block_start_bp-$block_end_bp|awk '{print $sample\" \"\$0}'|sed -r 's/\\s/\t/g'";
			my $rg_depth=`$cmd`;
			die "error:$cmd\n" if($?);
		}else{
